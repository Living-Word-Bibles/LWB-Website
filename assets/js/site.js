(() => {
  ...
  const RUNTIME_VERSION = '2026-09-07.2';
  ...
  const PRIVACY_NOTICE_VERSION = '2026-09-07';
  const PRIVACY_SESSION_KEY = 'lwbPrivacyConsentSession';
  ...
  function privacySessionId() {
    try {
      let value = sessionStorage.getItem(PRIVACY_SESSION_KEY) || '';
      if (!value) {
        value = randomClientId('pcs');
        sessionStorage.setItem(PRIVACY_SESSION_KEY, value);
      }
      return value;
    } catch (_) {
      return randomClientId('pcs');
    }
  }

  function analyticsVisitorId(createIfMissing) {
    try {
      let value = localStorage.getItem(ANALYTICS_VISITOR_KEY) || '';
      if (!value && createIfMissing) {
        value = randomClientId('v');
        localStorage.setItem(ANALYTICS_VISITOR_KEY, value);
      }
      return value;
    } catch (_) {
      return createIfMissing ? randomClientId('v') : '';
    }
  }

  function safeCurrentPath() {
    try { return new URL(location.href).pathname || '/'; }
    catch (_) { return String(location.pathname || '/').split('?')[0].split('#')[0]; }
  }

  function necessaryApiPost(action, details) {
    const apiUrl = window.LWB_SITE_CONFIG?.apiBase || '';
    if (!apiUrl) return false;

    let accountToken = '';
    let adminToken = '';
    try { accountToken = localStorage.getItem(ACCOUNT_SESSION_KEY) || ''; } catch (_) {}
    try { adminToken = localStorage.getItem(PORTAL_SESSION_KEY) || ''; } catch (_) {}
  ...
  function logPrivacyChoice(previousStored, saved, source) {
    const previous = previousStored || null;
    let eventType = 'PRIVACY_CONSENT';

    if (previous) {
      const withdrew =
        (previous.analytics === true && saved.analytics === false) ||
        (previous.advertising === true && saved.advertising === false);
      eventType = withdrew ? 'PRIVACY_CONSENT_WITHDRAWN' : 'PRIVACY_PREFERENCES_UPDATED';
    }

    necessaryApiPost('privacy-consent', {
      event_type: eventType,
      event_id: randomClientId('consent'),
      necessary: true,
      analytics: saved.analytics === true,
      advertising: saved.advertising === true,
      consent_version: saved.version,
      notice_version: PRIVACY_NOTICE_VERSION,
      choice_source: source || 'unknown',
      previous_analytics: previous ? previous.analytics === true : null,
      previous_advertising: previous ? previous.advertising === true : null,
      client_time: new Date().toISOString(),
      path: safeCurrentPath(),
      session_id: privacySessionId(),
      visitor_id: saved.analytics === true
        ? analyticsVisitorId(true)
        : (previous?.analytics === true ? analyticsVisitorId(false) : '')
    });
  }

  function productContext() {
    const path = safeCurrentPath();
    if (path === '/estore/' || path === '/estore') {
      return {
        slug: 'estore-paid-digital-products',
        title: 'Living Word Bibles eStore Paid Digital Products'
      };
    }

    const card = document.querySelector('[data-product-card][data-product-slug]');
    const slug =
      card?.getAttribute('data-product-slug') ||
      path.replace(/^\/+|\/+$/g, '').split('/').pop() ||
      '';
    const title =
      card?.querySelector('[data-product-title]')?.textContent?.trim() ||
  ...
  async function complianceCountry() {
    try {
      ...
      return {
        country_code: String(data.country_code || '').toUpperCase().slice(0, 3),
        country_name: String(data.country_name || '').slice(0, 80),
        region: String(data.region || '').slice(0, 100),
        region_code: String(data.region_code || '').slice(0, 20)
      };
    } catch (_) {
      return { country_code:'', country_name:'', region:'', region_code:'' };
    }
  }

  function initializeEuEeaConsentLogging() {
    const checkbox = document.querySelector('#eu-eea-withdrawal-consent, [data-eu-eea-withdrawal-consent]');
    if (!checkbox || checkbox.dataset.lwbConsentLogReady === RUNTIME_VERSION) return;

    checkbox.dataset.lwbConsentLogReady = RUNTIME_VERSION;

    checkbox.addEventListener('change', async () => {
      const context = productContext();
      const locationData = await complianceCountry();

      necessaryApiPost('eu-eea-digital-consent', {
        event_id: randomClientId('euconsent'),
        consented: checkbox.checked === true,
        consent_version: EU_EEA_CONSENT_VERSION,
        consent_text: EU_EEA_WITHDRAWAL_TEXT,
        client_time: new Date().toISOString(),
        path: safeCurrentPath(),
        page_url: location.origin + safeCurrentPath(),
  ...
  function initializePrivacyCenter() {
    ...
    const commit = (choices, source) => {
      const previousStored = readPrivacyChoices();
      const previous = previousStored || defaultPrivacyChoices();
      const saved = writePrivacyChoices(choices);

      logPrivacyChoice(previousStored, saved, source);
    ...
    if (banner.dataset.lwbPrivacyReady !== RUNTIME_VERSION) {
      ...
      banner.querySelector('[data-lwb-privacy-accept]')?.addEventListener('click', () => {
        commit({ analytics: true, advertising: true }, 'banner-accept-all');
      });

      banner.querySelector('[data-lwb-privacy-reject]')?.addEventListener('click', () => {
        commit({ analytics: false, advertising: false }, 'banner-reject-optional');
      });
    ...
    if (backdrop.dataset.lwbPrivacyReady !== RUNTIME_VERSION) {
      ...
      backdrop.querySelector('[data-lwb-privacy-modal-reject]')?.addEventListener('click', () => {
        ...
        commit({ analytics: false, advertising: false }, 'settings-reject-optional');
      });
